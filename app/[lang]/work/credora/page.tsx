...
const route = "/work/credora";

export async function generateMetadata({
  params,
}: PageProps<"/[lang]/work/credora">): Promise<Metadata> {
  const { lang } = await params;
  if (!isLocale(lang)) notFound();

  const project = getSideProject(lang, "credora");
...
export default async function CredoraPage({ params }: PageProps<"/[lang]/work/credora">) {
  const { lang } = await params;
  if (!isLocale(lang)) notFound();
  ...
  return (
    <SideProjectPage
      locale={lang}
      project={getSideProject(lang, "credora")}
      next={{ title: getSideProject(lang, "kiseki-no-oto").name, href: "/work/kiseki-no-oto" }}
    />
  );
}
